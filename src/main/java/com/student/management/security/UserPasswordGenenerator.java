...
public class UserPasswordGenenerator {
	private static final Random rndmMethod = new Random();
	private static final Logger logger = LoggerFactory.getLogger(UserPasswordGenenerator.class);

	private UserPasswordGenenerator() {
	...
	public static String getPassword(int len) {
		...
		logger.error("Your new password is : ");
		...
		for (int i = 0; i < len; i++) {
			/*
			 * Use of charAt() method : to get character value Use of nextInt() as it is
			 * scanning the value as int
			 */
	...
	public static String getOTP(int len) {
		...
		// Using random method
		/**
		 * private final Random rndmMethod = new Random();
		 */
